...
app.add_middleware(
    CORSMiddleware,
    allow_origins=[WEB_APP_URL.rstrip("/")],
...
def verify_init_data(init_data: str) -> dict:
    ...
    if auth_date and time.time() - auth_date > 86400:
        raise HTTPException(
            401,
            "انتهت صلاحية جلسة Telegram، أعد فتح التطبيق"
        )
...
def build_order_message(order, user):
    return (
        ...
        f"🆔 <b>رقم الطلب:</b> "
        f"<code>{html_escape(order['order_id'])}</code>\n"

        f"👤 <b>الطالب:</b> "
        f"{html_escape(user.get('first_name'))} "
        f"(@{html_escape(user.get('username') or 'بدون_يوزر')})\n"

        f"🛠️ <b>الخدمة:</b> "
        f"{html_escape(order['service_name'])}\n"

        f"🏫 <b>التخصص:</b> "
        f"{html_escape(order['department'])}\n"

        f"📌 <b>العنوان:</b> "
        f"{html_escape(order['title'])}\n"

        f"📄 <b>الصفحات:</b> "
        f"{html_escape(order['page_count'])}\n"

        f"🌐 <b>اللغة:</b> "
        f"{html_escape(order['language'])}\n"

        f"⏰ <b>الموعد:</b> "
        f"{html_escape(order['deadline'])}\n"

        f"📝 <b>الملاحظات:</b> "
        f"{html_escape(order['notes'])}"
    )
    ...
    try:
        ...
        if attachment_token:
            ...
            if attachment:
                with db() as conn:
                    conn.execute(
                        "DELETE FROM attachments WHERE token=%s",
                        (attachment_token,)
                    )
        ...
        notification = build_order_message(
            saved,
            {
...
def read_root():
    return {
        "status": "Online",
        "system": "النبع للخدمات الجامعية API",
...
async def startup_event():
    ...
    # Webhook فقط — لا يتم تشغيل getUpdates / Polling
    if WEBHOOK_URL:
